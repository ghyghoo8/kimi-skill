...
- `kimi vis [sessionId]`（v0.16）：浏览器里直观回看一次会话全过程（只读可视化）。
- `kimi web` / `/web`（v0.17）：把当前会话**挪到浏览器继续操作**；背后是 `kimi server` 本地服务（默认端口 `58627`）。细节见 `cli-reference.md`。

### 额外工作目录（v0.19）

`/add-dir <path>`（会话内）或 `kimi --add-dir <path>`（启动，可重复）添加工作目录外的目录；选「记住」写入项目级 `.kimi-code/local.toml`。
