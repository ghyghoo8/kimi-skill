...
- v0.23.5 起，供应商 429 / 过载等瞬时错误的**重试事件**也会出现在 stream-json 中；不要把它们误当最终回复。
- v0.24.2 起，后台任务完成可 steer 主 Agent 进入多个后续 turn，因此一次 `-p` 可能产生多组 Assistant / Tool 消息；不带 `tool_calls` 的、最后一条 Assistant 消息通常才是**最终结论**。
...
#!/usr/bin/env bash
set -o pipefail
out="$(kimi -p "$1" --output-format stream-json 2>/dev/null)"
code=$?
...
proc = subprocess.run(
    ["kimi", "-p", task, "--output-format", "stream-json", "-w", workdir],
...
> `-p` 下用 `/goal <目标>` 创建目标；v0.24.2 起进程会保持运行直到目标完成、受阻或暂停，再以 `0`/`3`/`6` 退出（详见 `kimi-cli/references/interaction.md`）。
...
若版本不直接暴露 id，可改用 `-c`（续接当前目录最近会话；`-C` 仅为隐藏旧别名）实现多轮。
